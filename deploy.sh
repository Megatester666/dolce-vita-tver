...
# ============================================================================
# Скрипт автоматического развертывания Dolce Vita Shop на VPS
# ============================================================================
...
show_welcome() {
   ...
   ╔═══════════════════════════════════════════════════════════╗
   ║                                                           ║
   ║       🛍️  DOLCE VITA SHOP - СКРИПТ РАЗВЕРТЫВАНИЯ 🛍️         ║
   ║                                                           ║
...
get_user_input() {
    ...
    # Название приложения
    echo -e "\n${BOLD}${YELLOW}Введите название магазина (по умолчанию: Dolce Vita Shop):${NC}"
    read -p "> " APP_NAME
    APP_NAME=${APP_NAME:-"Dolce Vita Shop"}
...
services:
  ...
  frontend:
    build:
      ...
      args:
        - VITE_API_BASE_URL=https://$DOMAIN/api
    container_name: dvshop_frontend
    depends_on:
      - backend
    expose:
      - "80"
...
build_and_run_docker() {
    ...
    if docker ps -a | grep -q "dvshop"; then
